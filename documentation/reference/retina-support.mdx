...
To disable retina support set the `devicePixelRatio` parameter to 1.0 (see [query parameters documentation](/documentation/usage/parameters/)). `devicePixelRatio` acts as a multiplier on the width and height of your chart. Fractional values are supported: a 400×200 chart with `devicePixelRatio=1.5` produces a 600×300 image. The maximum ratio is 2.

## How to display retina images

Most modern smart phones, tablets, and laptops have high-DPI displays. To let the browser choose between normal and retina images, use the `srcset` attribute on your `<img>` tag. This attribute allows you to specify multiple image URLs and their corresponding pixel densities. The browser will automatically choose the best image for the user's device.
