...
<CodeWithHighlights
  wrap
  code="https://quickchart.io/qr?**text**=Here's%20my%20text&**dotStyle**=rounded&**finderStyle**=circle&**finderColor**=ff0066"
/>

<img
  loading="lazy"
  src="https://quickchart.io/qr?text=Here's%20my%20text&dotStyle=rounded&finderStyle=circle&finderColor=ff0066"
/>

<Admonition type="tip">
  Styled QR codes are still decodable by standard QR readers, but you should always scan-test your
  final design. Highly stylized codes may be harder to read at small sizes or in low contrast.
</Admonition>
...
<CodeWithHighlights wrap code="https://quickchart.io/qr?text=https%3A%2F%2Fexample.com&format=svg&size=240&caption=Scan%20me%0AVisit%20our%20website" />

<img loading="lazy" alt="QR code with a two-line caption" src="https://quickchart.io/qr?text=https%3A%2F%2Fexample.com&format=svg&size=240&caption=Scan%20me%0AVisit%20our%20website" />
...
For JSON errors instead of an image, check the same body with [`/api/validate-qr`](/documentation/apis/agent-friendly-api/#validate-a-qr-code-config).
...
These endpoints encode the parameters into URLs. They do not render, validate, or save the QR codes. Any `key` included in the body will appear in the returned URL; use [signed URLs](/documentation/authentication/#signing-requests) if you need to share authenticated images.

## Dynamic QR codes

QR codes from `/qr` are static: the text is encoded directly in the image, so changing a URL means reprinting. If you need to change a destination after printing, pause a code, or track scans, use [dynamic QR codes](/documentation/qr-codes/dynamic-qr-codes/). They're managed through a [dashboard](https://qr.quickchart.io) and API and are included with every QuickChart API key.

## Bulk QR code generator

To generate many QR codes in a single request and receive them as a ZIP archive, use the [QR Code Batch API](/documentation/qr-codes/qr-batch-api/).

If you're looking to generate codes quickly without writing code, try our web-based [bulk QR code generator](https://quickchart.io/bulk-qr-code-generator/).

To create up to 10,000 editable, trackable codes from a CSV, use [dynamic QR bulk creation](/documentation/qr-codes/dynamic-qr-codes/#bulk-creation).
